import { buildPreNotificationTrigger, buildNotificationTrigger } from '../utils/scheduleUtils';
...
describe('buildPreNotificationTrigger', () => {
  it('computes daily pre-notice hour/minute correctly when offset does not borrow an hour', () => {
    const t = buildPreNotificationTrigger('09:10', 'daily', undefined, undefined, 5) as any;
  ...
  it('computes daily pre-notice that borrows hour when minutes < offset', () => {
    const t = buildPreNotificationTrigger('09:02', 'daily', undefined, undefined, 5) as any;
    expect(t.repeats).toBe(true);
  ...
  it('computes absolute date pre-notice for once', () => {
    ...
    const pre = buildPreNotificationTrigger('09:00', 'once', now, undefined, 5) as any;
    const mainDate = (main as any).date as Date;
    const preDate = pre.date as Date;
    expect(Math.round((mainDate.getTime() - preDate.getTime()) / 60000)).toBe(5);
  });
});
